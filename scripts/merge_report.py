...
- `sysbench` 用 `--threads=1`（单线程，对照「单核 CPU」）且**显式钉住 `--cpu-max-prime=10000`**；`7z b` 默认吃满所有线程（多线程，对照「多核 CPU」）。两者量纲与负载模型不同，不要互相换算。
- ⚠️ `--cpu-max-prime` 必须钉住才可比：该参数直接决定 events/s 的量级（实测 100 → 247 万、1000 → 11.9 万、10000 → 4435），而各平台默认值并不一致。修好之前的一次运行里 macOS 报出 8,150,296 而 Linux 是 3,672，差 2200 倍，纯粹是参数差异。**引用历史运行数据时请先核对 `sysbench_cpu_args`。**\
"""
